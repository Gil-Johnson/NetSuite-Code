var Constants = {
    api_url: "https://forms.netsuite.com/app/site/hosting/scriptlet.nl?script=70&deploy=1&compid=3500213&h=96455628d6e6e4c09126",
    Filters: {
    	teams: 'team',
    	leagues: 'league',
    	prodTypes: 'prod-type'
				...
				if (!!$('#league') && $('#league').length > 0) {
					$('#league').multiselect({
						close : function() {
                            ...
                            //Enable/disable the Get results button based on selected values
                            //disableGetresultsBtn();
...
function downloadImages(){
    ...
    if(!!selectedRows && selectedRows.length > 0) {
        ...
        $.ajax({
            url: Constants.download_initializing_url,
            type: "POST",
            data: {
...
function checkDownloadStatus(postData) {
    ...
    if(eta == "0") {
        ...
        setTimeout(function() {
            $.ajax({
                url: Constants.download_status_check_url,
                type: "POST",
                data: {
                    auth_token: Constants.auth_token,
                    folderName: uid

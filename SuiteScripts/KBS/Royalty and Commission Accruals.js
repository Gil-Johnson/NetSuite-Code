...
try
{
	if (type == 'create' || type == 'edit')
	{
		...
		for(z=1;z<=size;z++) 
		{
//			var rrate1 = nlapiGetLineItemValue('item', 'custcol_royaltypercent', z)
//			if (!rrate1)
//			{
...
//			}
			...
			var liccsor1 = nlapiGetLineItemValue('item', 'custcol_hiddenlicensorfield', z)
			if (!liccsor1)
			{
...
//			}
		...
		if (errorfound)
		{
			...
			var ccaddr = 'ffoster@grace-solutions.com';  

			var emailbody = "Failure on Roayalty or Commmision Calculation Routine: " + today + "\r\n" + errormessage
			nlapiSendEmail(fromaddr, toaddr, 'Roayalty or Commmision Calculation Routine', emailbody,ccaddr, null);
			nlapiLogExecution('Error','Roayalty or Commmision Calculation Routine',errormessage);
			return;
		}
		...
		for(x=1;x<=size;x++) 
		{
			...
			var itemflds = nlapiLookupField('item', itemid, ['custitem2', 'custitemcustitem_team2', 
				'custitem_nondistrlty1', 'custitem_distrlty1']) //Team & Royalty
			...
			var league = nlapiGetLineItemValue('item', 'custcol_league', x)
...
//			var accrulicensor = nlapiGetLineItemValue('item', 'custcol_accrual_licensor', x)
			...
			royaltyrec.setLineItemValue('line', 'league', lineno, league)
			...
			if (itemflds.custitem2)
			{
				royaltyrec.setLineItemValue('line', 'custcol_team', lineno, itemflds.custitem2)//Team 1
			}
			if (liccsor1) 
			{
			...
			if (itemflds.custitemcustitem_team2)
			{
				royaltyrec.setLineItemValue('line', 'custcol_team', lineno, itemflds.custitemcustitem_team2) //Team 2
			}
...
//			var commclass = nlapiGetLineItemValue('item', 'custcol_commpercent', x)
			...
			if (itemflds.custitemcustitem_team2)
			{
				commrec.setLineItemValue('line', 'custcol_team', lineno_comm, itemflds.custitemcustitem_team2) //Team
			}

...
var CommonDAL = (function () {
    ...
    return {
        ...
        getItems: function (options) {
            ...
            var substituteBuildableQuantity = new nlobjSearchColumn('formulanumeric').setFormula('{custitem_substitute.custitem_buildableqty}');
            var substituteArtStatus = new nlobjSearchColumn('formulatext').setFormula('{custitem_substitute.custitem_status}');
            var substituteDescription = new nlobjSearchColumn('formulatext').setFormula('{custitem_substitute.salesdescription}');
            ...
            var subinternalImageCol = new nlobjSearchColumn('formulatext').setFormula("CASE \nWHEN {isinactive} = 'T' OR {isinactive} = 'Yes' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:red;font-size: 24pt;font-family: Helvetica;\">Inactive</div>' \nWHEN {custitem_substitute.custitem_discontinued} = 'T' OR {custitem_substitute.custitem_discontinued} = 'Yes' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:red;font-size: 24pt;font-family: Helvetica;\">Discontinued</div>' \nWHEN  {custitem_substitute.custitem_status} = 'On Hold' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:red;font-size: 24pt;font-family: Helvetica;\">' || {custitem_substitute.custitem_status} || '</div>' \nWHEN  {custitem_substitute.custitem_status} = 'Canceled' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:red;font-size: 24pt;font-family: Helvetica;\">' || {custitem_substitute.custitem_status} || '</div>' \nWHEN  {custitem_substitute.custitem_status} = 'Approved' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:blueviolet;font-size: 24pt;font-family: Helvetica;\">' || {custitem_substitute.custitem_status} || '</div>' \nWHEN  {custitem_substitute.custitem_status} = 'Completed' THEN '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:forestgreen;font-size: 24pt;font-family: Helvetica;\">' || {custitem_substitute.custitem_status} || '</div>' \nELSE '<img src=\"'||{custitem_substitute.custitemthumbnail_image}||'\">'|| '<div style=\"color:black;font-size: 24pt;font-family: Helvetica;\">' || {custitem_substitute.custitem_status} || '</div>' \nEND");
            ...
            cols.push(subinternalImageCol);
            cols.push(substituteArtStatus);
            cols.push(substituteBuildableQuantity);
            cols.push(substituteDescription);
            ...
            var result = baseTypeDAL.getObjects(records).map(function(r) {
                ...
                if (r.custitem_substitute) {
                    ...
                    r.custitem_internalimagethumb_sub = record.getValue(subinternalImageCol) || '';                    
                    r.custitem_substitute.artstatus = record.getValue(substituteArtStatus)|| '';                    
                    r.custitem_substitute.buildablequantity = record.getValue(substituteBuildableQuantity);                    
                    r.custitem_substitute.description = record.getValue(substituteDescription)|| '';
                }

...
function sendStatement(type) {
    ...
    try {
        ...
        cuscolumns[5] = new nlobjSearchColumn('custentity_email_statement_pref');
        ...
        for (var i = 0; customers != null && i < customers.length; i++) {       
            ...
            var customerStatementPref = customer.getValue('custentity_email_statement_pref'); //PDF is value 2
			...
			//id pref is df create pdf and attach it
			if(customerStatementPref == 2){
				var sdate = new Array();
				sdate.startdate = '02/07/2008';
				sdate.statementdate = today;
				sdate.openonly = 'T';
				
				//print the statement to a PDF file object
				file1 = nlapiPrintRecord('STATEMENT', customerId, 'PDF', sdate);
				emailBody2 = emailBody1.replace('{{itmhtml}}', "");
		  ...
		  } // end else statment on customer prefernce 
            ...
            try {
				...
				nlapiSendEmail(18099, recipients, emailSubject, emailBody2, ccemails, null, rec, file1);

...
var SalesOrdersDAL = Fiber.extend(function () {
    ...
    return {
        ...
        getPending: function (params) {
            ...
            if (params.lastorder && params.lastorder.length > 0){
            	F3.Util.Utility.logDebug('orders',params.lastorder);
            	var excludeOrders = params.lastorder.split(",");
            	F3.Util.Utility.logDebug('oarray',excludeOrders.toString());
                filters.push(new nlobjSearchFilter('internalid', null, 'noneof', excludeOrders));
            }

            
            F3.Util.Utility.logDebug('orders2',params.lastorder);

...
var ToolingScheduled = (function () {
    ...
    return {
        scheduled: function (type) {
			...
			if(context.getSetting('SCRIPT', 'custscript_count1'))
				countInserted = parseInt(context.getSetting('SCRIPT', 'custscript_count1'));
		...
		processRecords: function(records) {
			...
			for (i in records) {		
				try {
					...
					nlapiLogExecution('DEBUG', 'Tooling Number', toolingNumber );
					if (toolingNumber) {		
                                                 nlapiLogExecution('DEBUG', 'Here');
						var record = nlapiLoadRecord('customrecord_tooling', toolingNumber);
						record.setFieldValue('custrecord_ordered', 'T');
